package com.yilan.distributed.sequence.zk;

import com.yilan.distributed.lock.zk.ZkReentrantLockCleanerTask;
import com.yilan.distributed.sequence.DistributedSequence;
...
public class ZkDistributedSequence implements DistributedSequence {
    ...
    public Long sequence(String sequenceName) {
        try {
            int value=client.setData().withVersion(-1).forPath("/"+sequenceName,"".getBytes("UTF-8")).getVersion();
